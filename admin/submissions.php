...
<dialog id="preview-<?= $sub['sub_id'] ?>" class="rounded-2xl shadow-2xl w-full max-w-md p-0 overflow-hidden backdrop:bg-black/60" dir="rtl">
  ...
  <div class="p-6">
    ...
    <div class="bg-gray-50 rounded-2xl p-5 flex flex-col items-center text-center gap-3">
      ...
      <?php if (($data['p_nationality'] ?? null) || ($data['p_residence'] ?? null)): ?>
      ...
      <?php if ($bio): ?>
      <p class="text-gray-600 text-sm leading-7 text-center bg-white rounded-xl px-4 py-3 w-full"><?= nl2br(htmlspecialchars(mb_substr($bio,0,300))) ?><?= mb_strlen($bio)>300?'...':'' ?></p>
      <?php endif; ?>
    </div>
