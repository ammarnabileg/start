...
if ($method === 'GET' && $id === 'stats') {
    Response::success([
        'total_tenants'       => (int)$db->fetchColumn("SELECT COUNT(*) FROM tenants"),
        'active_tenants'      => (int)$db->fetchColumn("SELECT COUNT(*) FROM tenants WHERE status='active'"),
        'total_users'         => (int)$db->fetchColumn("SELECT COUNT(*) FROM users WHERE is_super_admin=0"),
        'total_applications'  => (int)$db->fetchColumn("SELECT COUNT(*) FROM applications"),
        ...
        'total_tokens'        => (int)$db->fetchColumn("SELECT COALESCE(SUM(total_tokens),0) FROM ai_usage_logs"),
    ]);
}
...
if ($method === 'GET' && $id === 'users' && !$sub) {
    ...
    $result = $db->paginate(
        "SELECT u.id, u.email, u.first_name, u.last_name, u.status, u.last_login_at, t.name as tenant_name
...
// PATCH /api/v1/super/users/{id}/status
if ($method === 'PATCH' && $id === 'users' && $sub && $sub2 === 'status') {
    $status = $req->input('status','');
    if (!in_array($status,['active','inactive'])) Response::error('Invalid status');
    $db->update('users', ['status'=>$status], ['id'=>(int)$sub]);
...
if ($method === 'GET' && $id === 'ai-usage') {
    $perTenant = $db->fetchAll(
        "SELECT t.name as tenant_name, l.tenant_id,
                SUM(l.total_tokens) as total_tokens, COUNT(*) as total_calls
         FROM ai_usage_logs l JOIN tenants t ON t.id=l.tenant_id
         GROUP BY l.tenant_id, t.name ORDER BY total_tokens DESC"
    );
    Response::success($perTenant);
}
