...
class ContentApiController
{
    ...
    private function getBrand(string $id): array {
        $b=$this->brandModel->find($id); if(!$b){$this->res->error('Brand not found.',404);exit;}
    ...
    public function index(array $p): void {
        ...
        $this->res->paginated($r);
    }
    public function store(array $p): void {
        $b=$this->getBrand($p['brandId']); $d=$this->req->all();
        $c=$this->contentModel->create(array_merge($d,['brand_id'=>$b['id'],'created_by'=>Auth::id()]));
        $this->res->success($c,'Content created.',201);
    }
    public function show(array $p): void {
        $b=$this->getBrand($p['brandId']); $c=$this->contentModel->find($p['id']);
        if(!$c||$c['brand_id']!=$b['id']){$this->res->error('Not found.',404);return;} $this->res->success($c);
    }
    public function update(array $p): void {
        $b=$this->getBrand($p['brandId']); $c=$this->contentModel->find($p['id']);
        if(!$c||$c['brand_id']!=$b['id']){$this->res->error('Not found.',404);return;}
        $this->contentModel->update($p['id'],$this->req->all()); $this->res->success($this->contentModel->find($p['id']));
    }
    public function delete(array $p): void {
        $b=$this->getBrand($p['brandId']); $this->contentModel->delete($p['id']); $this->res->success([],'Deleted.');
    }
    public function approve(array $p): void {
        $b=$this->getBrand($p['brandId']);
        if(!$this->brandModel->userCanAccess($b['id'],Auth::id(),'manager','admin','owner')){$this->res->error('Insufficient permissions.',403);return;}
        if($this->contentModel->approve($p['id'],Auth::id())) $this->res->success([],'Approved.'); else $this->res->error('Cannot approve.');
    }
}
