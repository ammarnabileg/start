...
<header class="bg-white dark:bg-[#1a1a1a] border-b border-gray-200 dark:border-white/10">
 ...
 <nav id="tabs-nav" class="flex items-center gap-0 overflow-x-auto scrollbar-hide cursor-grab select-none">
...
(function(){
 ...
 nav.addEventListener('mousedown', e => { isDown = true; nav.classList.add('cursor-grabbing'); startX = e.pageX - nav.offsetLeft; scrollLeft = nav.scrollLeft; });
 nav.addEventListener('mouseleave', () => { isDown = false; nav.classList.remove('cursor-grabbing'); });
 nav.addEventListener('mouseup', () => { isDown = false; nav.classList.remove('cursor-grabbing'); });
 nav.addEventListener('mousemove', e => { if (!isDown) return; e.preventDefault(); nav.scrollLeft = scrollLeft - (e.pageX - nav.offsetLeft - startX); });
 // Touch swipe
 let tx = 0;
 nav.addEventListener('touchstart', e => { tx = e.touches[0].pageX; scrollLeft = nav.scrollLeft; }, {passive:true});
 nav.addEventListener('touchmove', e => { nav.scrollLeft = scrollLeft - (e.touches[0].pageX - tx); }, {passive:true});
 // Scroll active tab into view
 const active = nav.querySelector('.border-brand');
...
<div class="space-y-5">
 ...
 <div class="flex gap-2">
 <button onclick="approveMember(<?= $pm['membership_id'] ?>, 'approved', this)" class="px-3 py-1.5 bg-teal-500/20 text-teal-400 border border-teal-500/30 rounded-lg text-xs font-semibold hover:bg-teal-500/30 transition-colors">✓ Approve</button>
 <button onclick="approveMember(<?= $pm['membership_id'] ?>, 'rejected', this)" class="px-3 py-1.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg text-xs font-semibold hover:bg-red-500/20 transition-colors">✕ Reject</button>
 </div>
 </div>
 <?php endforeach; ?>
 </div>
 ...
 <p class="text-sm text-gray-600 py-2">No topics yet.</p>
 <?php endif; ?>
 </div>
 <div class="flex gap-2">
