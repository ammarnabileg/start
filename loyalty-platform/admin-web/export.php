...
if ($type === 'users') {
  ...
  audit('export','users',null,['rows'=>count($rows)]);
  stream_csv('users.csv', ['ID','الاسم','الجوال','البريد','الانضمام','عدد المتاجر','إجمالي النقاط'],
    array_map(fn($r)=>[$r['id'],$r['name'],$r['phone'],$r['email'],$r['created_at'],$r['stores'],$r['points']], $rows));
}
...
if ($type === 'merchants') {
  ...
  audit('export','merchants',null,['rows'=>count($rows)]);
  stream_csv('merchants.csv', ['النشاط','النوع','الجوال','البريد','الحالة','الانضمام','العملاء'],
    array_map(fn($r)=>[$r['business_name'],$r['business_type'],$r['phone'],$r['email'],$r['status'],$r['created_at'],$r['customers']], $rows));
}
...
if ($type === 'reports') {
  ...
  audit('export','reports',null,['rows'=>count($rows)]);
  stream_csv('reports.csv', ['التاريخ','الحالة','المستخدم','الجوال','التاجر','الرسالة'],
    array_map(fn($r)=>[$r['created_at'],$r['status'],$r['uname'],$r['phone'],$r['business_name'],$r['message']], $rows));
}
