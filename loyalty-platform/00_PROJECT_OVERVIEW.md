...
| `spin-wheel` | العميل يلفّ عجلة الحظ — السيرفر يخصم النقاط ويختار النصيب (weighted) ويحفظ الهدية | تطبيق العميل |
| `redeem-prize` | الكاشير يفعّل هدية — يتحقق من QR متغيّر + صلاحية الموظف + نطاق الفرع | تطبيق التاجر |
| `claim-staff` | ربط حساب الموظف الجديد بدعوته (بمطابقة رقم الجوال) | تطبيق التاجر |
| `apply-coupon` | يتحقق ويطبّق كوبون | تطبيق التاجر |

> **الصلاحيات (RBAC):** `add-points`/`record-visit`/`redeem-prize`/`send-announcement` تفرض `staff_can(staff, resource, action)` على السيرفر (owner يتجاوز). الواجهة تخفي البلاطات حسب صلاحية الموظف، لكن الفرض الحقيقي على الـ Edge Functions.
